...
│  ┌──────────────────────┐  │
│  │  📋 문답지             │  │
│  │  세렌시아 문답지 | 답변당 50P │ ← 각 답변마다 포인트 획득
│  │                      │  │
│  │  0/3 답변 완료         │  │ ← 진행률 표시 (질문 수는 유동적)
│  └──────────────────────┘  │
│                            │
│                            │
└────────────────────────────┘
...
**일기 쓰기 카드 클릭:**
- **미완료 상태** ("작성 가능 ○") → 일기 작성 페이지(`/guesthouse/:cityId/room/diary`)로 이동
- **완료 상태** ("작성 완료 ✓") → 작성한 일기 상세 페이지(`/diary/:diaryId`)로 이동 (수정 가능)

**문답지 카드 클릭:**
- 문답지 작성 페이지(`/guesthouse/:cityId/room/questionnaire`)로 이동
- 작성한 답변 조회 및 미작성 답변 작성 가능

**규칙:**
- **일기**: 체류당 1개 작성 (포인트: 체류당 1회 50P), 수정은 언제든 가능
- **문답지**: 1문 1답 구조, 질문별 개별 저장 (포인트: 답변당 50P), 수정은 언제든 가능
- 일기와 문답지 작성은 도시 체류 중일 때만 가능
- 일기와 문답지는 본인만 조회 가능
...
```
┌────────────────────────────┐
│ ← 뒤로  세렌시아 문답지  1/3 │ ← 질문 수는 DB에서 관리 (유동적)
├────────────────────────────┤
│                            │
│                            │
│  ┌──────────────────────┐  │
...
- 질문별로 개별 저장 (각 저장 시 50P 획득)
- 저장 후에도 해당 질문의 답변 수정 가능 (수정 시 포인트 미지급)
- 체류당 질문당 1개 답변만 가능

#### 도시별 문답지 질문 (DB에서 관리, 예시)
...
#### 저장 후
- "50포인트를 받았어요!" 토스트 (해당 질문 최초 답변 시에만)
- 다음 질문으로 이동 또는 개인 숙소로 복귀
- 수정 시에는 포인트 미지급
...
- 수정 버튼 → 해당 답변 수정 화면으로 이동
- 삭제 버튼 → 삭제 확인 후 삭제 (soft delete)
- 모든 질문과 답변을 세로로 표시 (질문 수는 유동적)
