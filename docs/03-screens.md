...
- **완료 상태** ("오늘 작성 완료 ✓") → 오늘 작성한 일기 상세 페이지(`/diary/:diaryId`)로 이동
- 상세 페이지에서 수정 가능 (체크인 후 24시간 이내)

**문답지 카드 클릭:**
- **미완료 상태** ("미완료 ○") → 문답지 작성 페이지(`/guesthouse/:cityId/room/questionnaire`)로 이동
- **완료 상태** ("완료 ✓") → 작성한 문답지 상세 페이지(`/questionnaire/:questionnaireId`)로 이동
- 상세 페이지에서 수정 가능

#### 제약사항
- room_stay에 체크인한 사용자만 접근 가능
- 일기는 체크인 후 24시간 이내 1회 작성/수정 가능
- 24시간 지나면 (연장한 경우) 새 일기 작성 가능

---

### 3.14 일기 작성/수정 (`/guesthouse/:cityId/room/diary`)
...
**작성 모드:**
- 빈 폼 표시
- 저장 시 "50포인트를 받았어요!" 토스트
- 개인 숙소로 복귀
- 하루 1회만 포인트 획득

**수정 모드:**
- 기존 일기 내용 불러오기 (제목, 본문, 기분)
- 저장 시 "일기가 수정되었어요" 토스트
- 이전 페이지로 복귀 (일기 상세 또는 개인 숙소)
- 포인트 추가 지급 없음

#### 제약사항
- 체크인 후 24시간 이내에만 작성/수정 가능
- 24시간 지나면 새 일기 작성 가능 (연장한 경우)
- 일기 본문 최대 500자

---

### 3.15 문답지 작성/수정 (`/guesthouse/:cityId/room/questionnaire`)
...
**작성 모드:**
- 해당 도시(city_id)의 질문들을 불러옴
- 빈 답변 폼 표시
- 마지막 질문 제출 후 "50포인트를 받았어요!" 토스트
- 개인 숙소로 복귀
- 도시별 1회만 포인트 획득

**수정 모드:**
- 해당 도시(city_id)의 질문들을 불러옴
- 사용자가 이전에 작성한 답변들을 불러와서 폼에 표시
- 저장 시 "문답지가 수정되었어요" 토스트
- 이전 페이지로 복귀 (문답지 상세 또는 개인 숙소)
- 포인트 추가 지급 없음 (has_earned_points로 관리)

#### 데이터 구조
- **질문 테이블(questions)**: id, city_id, question_1, question_2, question_3, created_at, updated_at
- **답변 테이블(answers)**: id, question_id, user_id, city_id, question_1_answer, question_2_answer, question_3_answer, has_earned_points, created_at, updated_at
...
- **[수정]** 버튼 (체크인 후 24시간 이내일 때만 표시) → 일기 작성/수정 페이지로 이동
- 24시간이 지난 일기는 읽기 전용
...
- **[수정]** 버튼 → 문답지 작성/수정 페이지로 이동 (이전 답변 불러오기)
...
- **"나의 일기"** 클릭 → 일기 리스트 페이지(`/diary`)로 이동
  - 작성한 일기 리스트 표시
  - 일기 클릭 시 상세 조회 가능
- **"나의 문답지"** 클릭 → 문답지 리스트 페이지(`/questionnaire`)로 이동
  - 작성한 문답지 리스트 표시
  - 문답지 클릭 시 상세 조회 가능
- "설정" 클릭 → 설정 화면 (Phase 2)
- "닫기" 또는 외부 클릭 → 모달 닫기
...
#### 기능
- 과거 일기 및 문답지 조회를 위한 진입점
- 일기/문답지는 체크인한 날짜별로 확인 가능
