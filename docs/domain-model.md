...
- **RoomDeleted**: 마지막 여행자 체크아웃 시 룸 삭제 (current_capacity = 0)
...
- **룸 삭제**: 마지막 여행자 체크아웃 시 (current_capacity = 0) 룸 삭제
...
3. **체크아웃 처리**:
   ...
   - 1:1 대화방 자동 삭제
   - Room의 current_capacity 감소
4. **룸 삭제**: 마지막 여행자 체크아웃 시 (current_capacity = 0) 룸 삭제
...
- **룸 생성/배정**: 동시 입장 시 Room 중복 생성 방지 (낙관적 락 또는 비관적 락)
- **룸 삭제**: 마지막 여행자 체크아웃 시 안전한 삭제 처리
- **포인트 거래**: 트랜잭션 격리 수준 설정 (Read Committed 이상)
- **개별 체크아웃**: 여러 여행자의 체크아웃이 동시에 발생해도 안전하게 처리

### 1-1. Room 생명주기 관리

- **생성 시점**: 첫 번째 여행자 체크인 시 Room 생성 (current_capacity = 1)
- **삭제 시점**: 마지막 여행자 체크아웃 시 Room 삭제 (current_capacity = 0)
- **삭제 방식**: Soft Delete (deleted_at 기록) 또는 Hard Delete (물리적 삭제)
  - Soft Delete 권장: 채팅 기록 참조 무결성 유지
  - 채팅 메시지는 3일 후 자동 삭제되므로, Room도 3일 후 완전 삭제 가능
- **채팅 메시지 처리**: Room 삭제 시 해당 룸의 채팅 메시지는 유지 (3일 보관 정책)

### 2. 성능 최적화

- **메시지 로딩**: 페이지네이션 (최근 50개)
- **메시지 삭제**: 배치 작업 (매일 자정)
- **인덱스**: ERD의 인덱스 정보 참조
- **캐싱**: 도시 정보, 대화 카드 등 자주 조회되는 데이터 캐싱
- **Rate Limiting**: Redis를 활용한 효율적인 요청 제한
  - Redis SET 명령어와 EXPIRE 사용
  - Lua 스크립트로 atomic 연산 보장

### 3. 보안

- **비밀번호**: bcrypt 해싱
- **개인정보**: 일기, 문답지, 1:1 대화 내용 보호
- **인증/인가**: JWT 토큰 기반 인증

### 4. 확장성

- **비동기 처리**: FastAPI async, SQLAlchemy async
- **메시지 큐**: Celery로 백그라운드 작업
  - 메시지 자동 삭제 (매일 자정)
  - 체크아웃 자동 처리
  - 체크아웃 1시간 전 알림
- **수평 확장**: 스테이트리스 API 설계
...
- **체크아웃 알림**:
  ...
  - 연장 시 기존 태스크 취소(revoke) 후 재예약
- **인앱 알림**: Phase 1
  - WebSocket 또는 SSE로 실시간 전송
  - 프론트엔드에서 토스트 메시지 표시
- **푸시/이메일 알림**: Phase 2에서 구현

### 6. 온보딩 자원 관리

- **이미지 저장**:
  - 경로: `/static/onboarding/`
  - 파일명: `story-1.webp`, `story-2.webp`, `story-3.webp`
  - 형식: WebP (최적화된 이미지 포맷)
  - 크기: 모바일 최적화 (1080x1920)
- **API 응답**:
  ```json
  {
    "images": [
      "/static/onboarding/story-1.webp",
      "/static/onboarding/story-2.webp",
      "/static/onboarding/story-3.webp"
    ]
  }
  ```
