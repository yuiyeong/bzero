...
2. **일기 작성/수정/조회**
    - 체크인 시간 기준 24시간 이내에 1회 작성 가능
    - 작성 후 읽기, 수정 가능
    - 체크인 후 24시간이 지나면 (연장한 경우) 새로운 일기 작성 가능
    - 이전 일기는 프로필 모달에서 확인 가능
    - 오늘 날짜 자동 표시
    - 제목 입력 (선택)
    - 본문 입력 (자유롭게, 500자 제한)
    - 기분 선택: 😊😐😢😠🥰 등
    - "저장" 버튼 → 50포인트 획득 (하루 1회만)
    - 프로필 모달의 일기 탭에서 과거 일기 리스트 조회 및 상세보기 가능
3. **문답지 작성/수정/조회**
    - 데이터 구조:
        - **질문 테이블(questions)**: id (PK), city_id (FK), question_1, question_2, question_3, created_at, updated_at
        - **답변 테이블(answers)**: id (PK), question_id (FK), user_id (FK), city_id (FK), question_1_answer, question_2_answer, question_3_answer, has_earned_points, created_at, updated_at
    - 작성 시 해당 도시(city_id)의 질문들을 불러옴
    - 수정 시 사용자가 이전에 작성한 답변들을 불러와서 수정 가능
    - 도시별 질문 3개 제공
    - 예) 세렌시아:
        - "요즘 나에게 힘이 되어주는 사람은?"
    ...
    - 이전/다음 버튼으로 질문 이동
    - 3개 모두 작성 후 "제출" → 50포인트 획득 (도시별 1회만)
    - has_earned_points 필드로 포인트 중복 지급 방지
    - 프로필 모달의 문답지 탭에서 과거 문답지 리스트 조회 및 상세보기 가능
...
- room_stay에 체크인한 사용자만 일기/문답지 작성 가능
- 일기는 하루에 1개만 포인트 획득 (체크인 후 24시간 주기)
- 문답지는 도시별로 1회만 포인트 획득
...
- [ ] 일기를 쓰고 저장할 수 있음
- [ ] 일기 작성 시 포인트가 지급됨 (하루 1회만)
- [ ] 일기 수정 및 조회 기능이 작동함
- [ ] 체크인 후 24시간 주기로 새 일기 작성이 가능함
- [ ] 도시별 문답지 질문 3개가 표시됨
- [ ] 문답지 작성 시 포인트가 지급됨 (도시별 1회만)
- [ ] 문답지 수정 및 조회 기능이 작동함
- [ ] 중복 포인트 지급이 방지됨 (has_earned_points)
- [ ] 일기와 문답지가 DB에 저장됨
- [ ] 프로필 모달에서 과거 일기/문답지 조회 가능
