...
지하 0층(B0)의 비행선 터미널에서 출발하여 테마별 도시를 방문하고, 게스트하우스에서 다른 여행자들과 만나 이야기를 나누며, 개인 숙소에서 일기와 문답지를 통해 자신을 돌아볼 수 있습니다.
...
> MVP에서는 **세렌시아**와 **로렌시아** 2개 도시만 오픈됩니다.
...
| 문서 | 설명 |
|------|-----|
| **[00-concept.md](docs/00-concept.md)** | 프로젝트 컨셉, 세계관, 5개 도시, 사용자 여정 |
| **[01-mvp.md](docs/01-mvp.md)** | MVP 개발 로드맵, 10개 기능 명세, 완료 조건 |
| **[02-design-system.md](docs/02-design-system.md)** | 색상 팔레트, 타이포그래피, 컴포넌트 스펙 |
| **[03-screens.md](docs/03-screens.md)** | 화면별 레이아웃, 와이어프레임, 상태 정의 |
...
핵심 기능 10개를 구현하여 정식 런칭:

| # | 기능 | 예상 시간 |
|---|-----|----------|
| 1 | 온보딩 & 회원가입 (Supabase + Backend) | 8h |
| 2 | B0 터미널 & 도시 선택 (2개) | 8h |
| 3 | 비행선 티켓 구매 (일반/쾌속) | 12h |
| 4 | 게스트하우스 (6명 룸 자동 배정) | 20h |
| 5 | 사랑방 (단체 채팅) | 15h |
| 6 | 라운지 (1:1 대화) | 12h |
| 7 | 개인 숙소 (일기, 문답지) | 8h |
| 8 | 포인트 시스템 | 6h |
...
- 나머지 3개 도시 오픈 (엠마시아, 다마린, 갈리시아)
- AI 호스트 (Ollama 기반 대화 촉진)
- 혼합형/조용한 방 구분
- 구조화된 이벤트 (불멍, 별멍 등)
- 과거 일기/문답지 조회
- 명상 기능
...
| 색상명 | HEX | 용도 |
|--------|-----|------|
| Brand Purple | `#9333EA` | 주요 버튼, CTA |
| Light Purple | `#C084FC` | 강조 텍스트, 링크 |
| Deep Navy | `#0F0F23` | 메인 배경 |
| Card Navy | `#1A1B3C` | 카드, 입력창 배경 |
...
자세한 내용은 [`docs/02-design-system.md`](docs/02-design-system.md)를 참조하세요.
